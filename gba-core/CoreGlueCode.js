"use strict";
 ...
 // LEGAL: the upstream project shipped a hardcoded catalog of ~104 commercial ROMs
// served from /binaries. That catalog and every ROM file has been removed.
// ROMs are now supplied by the user at runtime (File input / drag-drop) and are
// never fetched from, or stored on, this server.
var games = {};
...
window.onload = function() {
    ...
    registerGUISettings();
    // LEGAL: no catalog, so no hash-based game gate. Player starts idle awaiting a user file.
    //Download the BIOS:
    downloadBIOS();
}

function downloadBIOS() {
    // LEGAL: never ship or fetch Nintendo's GBA BIOS. iodineGBA boots HLE (SKIPBoot).
    // Optional: user may supply their OWN legally-dumped BIOS via attachBIOS().
    registerBIOS_HLE();
}
...
function downloadROM(gamename) {
    ...
    // LEGAL: removed server-side ROM delivery. ROMs arrive via user file input.
    throw new Error("Catalog ROM loading removed. Load a ROM from your own device.");
}
